class AddRecurreciesToUserEvents < ActiveRecord::Migration
  def change
    reversible do |dir|
      dir.up do
        execute <<-SQL
          DROP VIEW IF EXISTS complex_events;
          DROP FUNCTION IF EXISTS recurring_events_for(TIMESTAMP, TIMESTAMP, CHARACTER VARYING, INT);
      ...
      dir.down do
        execute <<-SQL
          DROP FUNCTION IF EXISTS recurring_events_for(INTEGER, INTEGER, TIMESTAMP, TIMESTAMP, CHARACTER VARYING, INT);
          CREATE OR REPLACE FUNCTION recurring_events_for(
            range_start TIMESTAMP,
            range_end  TIMESTAMP DEFAULT now(),
            time_zone CHARACTER VARYING DEFAULT NULL,
