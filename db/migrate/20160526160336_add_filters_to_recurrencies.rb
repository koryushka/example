class AddFiltersToRecurrencies < ActiveRecord::Migration
  def change
    reversible do |dir|
      dir.up do
        execute <<-SQL
          DROP VIEW IF EXISTS complex_events;
          DROP FUNCTION IF EXISTS recurring_events_for(INTEGER, TIMESTAMP, TIMESTAMP, CHARACTER VARYING, INTEGER);
          ...
          BEGIN
            FOR event IN
              SELECT *
                FROM user_events(current_user_id, events_filter)
                WHERE
                  frequency <> 'once' OR
                  (frequency = 'once' AND
      ...
      dir.down do
        execute <<-SQL
          DROP FUNCTION IF EXISTS recurring_events_for(INTEGER, TIMESTAMP, character varying, TIMESTAMP, CHARACTER VARYING, INT);
          ...
          BEGIN
            FOR event IN
              SELECT *
                FROM user_events(current_user_id)
                WHERE
                  frequency <> 'once' OR
                  (frequency = 'once' AND
                    ((starts_on IS NOT NULL AND ends_on IS NOT NULL AND starts_on <= (timezone('UTC', range_end) AT TIME ZONE time_zone)::date AND ends_on >= (timezone('UTC', range_start) AT TIME ZONE time_zone)::date) OR
