class Api::V1::EventCancellationsController < ApiController
  ...
  def create
    @event_calncellation = EventCancellation.new(event_cancellation_params)
    @event_calncellation.event = @event
    if @event_calncellation.valid?
      unless @event_calncellation.save
        return render nothing: true, status: :internal_server_error
      end
    else
      return render json: { validation_errors: @event_calncellation.errors.messages }, status: :bad_request
    end

    render partial: 'event_cancellation', locals: {event_cancellation: @event_calncellation }, status: :created
  end
